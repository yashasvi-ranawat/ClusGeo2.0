...
b=$(zenity 2>/dev/null --scale --text "Pic a buble size (2.x A). This will determine the surface. \n make sure to check if the surface was defined correctly afterwards. \n If too large, it might ignore valleys, if too small, it will create a bubble inside and think that there is a surface there." --min-value=1 --max-value=9 --value=7 --step 1)
case $? in
         0)
		echo "You selected l = $l.";;
         1)
...
if [ $nAtoms -eq 2 ];then
    ./surfaceTwo $FILE $atoms 0.$r0 $rcut $n $l $grid 2.$b  | tee "$FILE"_twoAt_0${r0}_${rcut}_${n}_${l}_"$grid".surfsoap | zenity --progress --auto-close
    ...
    --text="${FILE}_twoAt_0${r0}_${rcut}_${n}_${l}_$grid.surfsoap \nwas produced!"

 elif [ $nAtoms -eq 1 ]
    then
    ./surfaceOne $FILE 0.$r0 $rcut $n $l $grid 2.$b  | tee "$FILE"_oneAt_0${r0}_${rcut}_${n}_${l}_"$grid".surfsoap | zenity --progress --auto-close
    ...
    zenity 2>/dev/null  --info \
    --text="${FILE}_oneAt_0${r0}_${rcut}_${n}_${l}_$grid.surfsoap"
