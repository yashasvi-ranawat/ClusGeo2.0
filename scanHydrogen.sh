...
gridLevel=$(zenity 2>/dev/null --scale --text "Pic a grid density.\n O(N^3)" --min-value=50 --max-value=250 --value=150 --step 10)
case $? in
         0)
...
case $? in
         0)
		echo "You selected minimum distance = $minDist.";;
         1)
                echo "No value selected."; exit ;;
        -1)
                echo "An unexpected error has occurred."; exit;;
esac
maxDist=$(zenity 2>/dev/null --scale --text "Pic the closest hydrogen distance to surface.(2.x Ang)" --min-value=1 --max-value=9 --value=3 --step 1)
case $? in
         0)
		echo "You selected maximum distance = $maxDist.";;
         1)
                echo "No value selected."; exit ;;
        -1)
                echo "An unexpected error has occurred."; exit;;
...
case $? in
         0)
		echo "You selected sparseness = $spa.";;
         1)
                echo "No value selected."; exit ;;
        -1)
                echo "An unexpected error has occurred."; exit;;
